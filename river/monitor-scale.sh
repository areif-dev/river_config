...
for i in $(seq 0 ${#display_adapters[@]})
do 

  # Reached the end of the array, so end the loop
  if [ $i == ${#display_adapters[@]} ]; then
    break 
  fi 
  ...
  exec wlr-randr --output ${display_adapters[$i]} --scale $scale
done
